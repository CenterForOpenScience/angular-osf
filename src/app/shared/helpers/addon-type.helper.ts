...
export function isStorageAddon(addon: Addon | AuthorizedAddon | ConfiguredStorageAddonModel | null): boolean {
...
export function isCitationAddon(addon: Addon | AuthorizedAddon | ConfiguredStorageAddonModel | null): boolean {
  if (!addon) return false;
...
export function getAddonTypeString(addon: Addon | AuthorizedAddon | ConfiguredStorageAddonModel | null): string {
  if (!addon) return '';

  return isStorageAddon(addon) ? 'storage' : 'citation';
}

export function isAuthorizedAddon(addon: Addon | AuthorizedAddon | ConfiguredStorageAddonModel | null): boolean {
  if (!addon) return false;

  return addon.type === 'authorized-storage-accounts' || addon.type === 'authorized-citation-accounts';
}

export function isConfiguredAddon(addon: Addon | AuthorizedAddon | ConfiguredStorageAddonModel | null): boolean {
  if (!addon) return false;
